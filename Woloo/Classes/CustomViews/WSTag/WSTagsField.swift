...
open class WSTagsField: UIScrollView {
    ...
    open func addTag(_ tag: WSTag) {
        ...
        tagView.onDidRequestSelection = { [weak self] tagView in
            if let index = self?.tagViews.firstIndex(of: tagView) { // for custome selection handle.
...
extension WSTagsField {
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat, using closure: (_ tagView: WSTagView?, _ tagRect: CGRect?, _ textFieldRect: CGRect?) -> Void) {
        ...
        var totalHeight: CGFloat = Constant.WSTag.STANDARD_ROW_HEIGHT
        ...
        for tagView in tagViews {
            ...
            if curX + tagRect.width > maxWidth {
                // Need a new line
                curX = 0
                curY += Constant.WSTag.STANDARD_ROW_HEIGHT + spaceBetweenLines
                totalHeight += Constant.WSTag.STANDARD_ROW_HEIGHT
            }

            tagRect.origin.x = curX
            // Center our tagView vertically within STANDARD_ROW_HEIGHT
            tagRect.origin.y = curY + ((Constant.WSTag.STANDARD_ROW_HEIGHT - tagRect.height)/2.0)
        ...
        curX += max(0, Constant.WSTag.TEXT_FIELD_HSPACE - self.spaceBetweenTags)
        var availableWidthForTextField: CGFloat = maxWidth - curX

        if textField.isEnabled {
            var textFieldRect = CGRect.zero
            textFieldRect.size.height = Constant.WSTag.STANDARD_ROW_HEIGHT

            if availableWidthForTextField < Constant.WSTag.MINIMUM_TEXTFIELD_WIDTH {
                ...
                curX = 0 + Constant.WSTag.TEXT_FIELD_HSPACE
                curY += Constant.WSTag.STANDARD_ROW_HEIGHT + spaceBetweenLines
                totalHeight += Constant.WSTag.STANDARD_ROW_HEIGHT
                // Adjust the width
    ...
    private var maxHeightBasedOnNumberOfLines: CGFloat {
        ...
        return contentInset.top + contentInset.bottom + Constant.WSTag.STANDARD_ROW_HEIGHT * CGFloat(numberOfLines) + spaceBetweenLines * CGFloat(numberOfLines - 1)
    }
